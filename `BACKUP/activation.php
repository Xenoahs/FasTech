...
if(!isset($_GET['code']) OR !isset($_GET['user'])){
    ...
    $stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM customers WHERE activate_code=:code AND customer_id=:id");
    ...
    if($row['numrows'] > 0){
        if($row['status']){
            ...
            try{
                $stmt = $conn->prepare("UPDATE customers SET status=:status WHERE customer_id=:id");
                $stmt->execute(['status'=>1, 'id'=>$row['customer_id']]);
                $output .= '
